  ...
  return (
    <div className="grid grid-cols-2 gap-3">
      <label className="block">
        <span className="mb-1 block text-xs text-muted-foreground">{t("cascadingLocation.wilaya")}</span>
        <select
          value={wilayaId ?? ""}
          onChange={(e) => onChange(e.target.value ? Number(e.target.value) : null, null)}
          className="w-full rounded-xl bg-input px-3 py-3 text-sm"
        >
          <option value="">{t("cascadingLocation.select")}</option>
          {wilayas.map((w) => (
      ...
      <label className="block">
        <span className="mb-1 block text-xs text-muted-foreground">{t("cascadingLocation.baladiya")}</span>
        <select
          value={baladiyaId ?? ""}
          disabled={!wilayaId}
        ...
        >
          <option value="">{wilayaId ? t("cascadingLocation.select") : t("cascadingLocation.select_wilaya_first")}</option>
          {baladiyas.map((b) => (
