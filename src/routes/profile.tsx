...
function Page() {
  ...
  const TABS: { id: Tab; label: string }[] = [
    ...
    { id: "appointments", label: t("profile.tabs.appointments") },
  ...
  const cancelAppointment = async (apptId: string) => {
    ...
    toast.success(t("profile.toastCancelSuccess"));
  ...
  const removeFavorite = async (favId: string) => {
    ...
    if (error) { toast.error(error.message); return; }
    toast.success(t("profile.toastRemoveFavSuccess"));
  ...
  const saveProfile = async () => {
    ...
    setSaving(false);
    if (error) { toast.error(error.message); return; }
    toast.success(t("profile.toastSaveSuccess"));
    setEditing(false);
    load();
  };
  ...
  const onUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return;
    const { data: u } = await supabase.auth.getUser();
    if (!u.user) { toast.error(t("profile.toastLoginRequired")); return; }
    setUploading(true);
    try {
      ...
      toast.error(err.message ?? t("profile.toastPhotoError"));
    } finally {
      setUploading(false);
  ...
  const statusMap: Record<string, { label: string; cls: string }> = {
    pending: { label: t("profile.statusPending"), cls: "bg-amber-500/15 text-amber-600" },
    confirmed: { label: t("profile.statusConfirmed"), cls: "bg-green-500/15 text-green-600" },
    completed: { label: t("profile.statusCompleted"), cls: "bg-primary/15 text-primary" },
    cancelled: { label: t("profile.statusCancelled"), cls: "bg-destructive/15 text-destructive" },
  };
    ...
    <AppShell>
      <div dir="rtl" className="min-h-[100dvh] bg-background text-foreground">
        <div className="relative h-52">
          ...
          <div className="absolute top-4 left-4 right-4 flex justify-between items-center">
            <Link
              ...
              aria-label={t("profile.settingsAriaLabel")}
            >
            ...
            {!editing ? (
              ...
              >
                ...
                <span className="text-xs font-bold">{t("profile.editButton")}</span>
              </button>
            ) : (
              <div className="flex gap-2">
                ...
                >
                  ...
                  <span className="text-xs">{t("profile.saveButton")}</span>
                </button>
              </div>
            )}
          </div>
          ...
          <div className="absolute bottom-4 left-4 right-4 flex items-end gap-3">
            <div className="relative">
              ...
              <button
                onClick={() => fileRef.current?.click()}
                aria-label={t("profile.changePhotoAriaLabel")}
            ...
            <div className="flex-1 pb-1 text-right">
              <div className="flex items-center gap-2 justify-end">
                <h1 className="text-2xl font-black drop-shadow-lg">{profile?.full_name ?? t("profile.guestName")}</h1>
        ...
        <div className="mx-4 -mt-2 bg-card rounded-2xl border border-border p-4 grid grid-cols-3 gap-3 z-10 relative">
          {[
            { icon: Calendar, value: String(appointments.filter((a: any) => a.status !== "cancelled").length), label: t("profile.statsAppointments") },
            { icon: Heart, value: String(favorites.length), label: t("profile.statsFavorites") },
            { icon: Star, value: "0", label: t("profile.statsRatings") },
          ].map((s) => (
        ...
        <div className="mx-4 mt-3 grid grid-cols-2 gap-2">
          ...
          >
            ...
            <span className="text-sm font-medium">{t("profile.callButton")}</span>
          </a>
          <button
            ...
            {activeTab === "info" && (
              <>
                <section className="bg-card rounded-2xl border border-border p-4">
                  <h2 className="font-bold text-sm text-muted-foreground mb-3 text-right">{t("profile.bloodTypeSection")}</h2>
                  {editing ? (
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-[11px] text-muted-foreground">{t("profile.bloodTypeRegistered")}</p>
                      </div>
                    </div>
                  )}
                </section>
                ...
                <section className="bg-card rounded-2xl border border-border p-4">
                  <h2 className="font-bold text-sm text-muted-foreground mb-3 text-right">{t("profile.personalInfo")}</h2>
                  {editing ? (
                    <div className="space-y-3">
                      <div>
                        <label className="mb-1.5 block text-right text-xs text-muted-foreground">{t("profile.fullNameLabel")}</label>
                        <input
                      ...
                      <div>
                        <label className="mb-1.5 block text-right text-xs text-muted-foreground">{t("profile.phoneLabel")}</label>
                        <input
                          className="w-full rounded-xl px-4 py-3 text-right text-sm bg-input border border-border focus:outline-none focus:ring-2 focus:ring-primary"
                      ...
                      <div>
                        <label className="mb-1.5 block text-right text-xs text-muted-foreground">{t("profile.wilayaLabel")}</label>
                        <div className="relative">
                          <select
                    ...
                    <div className="space-y-3">
                      <InfoRow icon={Phone} label={t("profile.infoPhoneLabel")} value={profile?.phone ?? "—"} />
                      <Divider />
                      <InfoRow icon={MapPin} label={t("profile.infoWilayaLabel")} value={wilayaName} />
                      <Divider />
                      <InfoRow icon={Droplet} label={t("profile.infoBloodLabel")} value={profile?.blood_type ?? "—"} />
                    </div>
                  )}
                </section>
              </>
            ...
            {activeTab === "appointments" && (
              appointments.length === 0 ? (
                <section className="bg-card rounded-2xl border border-border p-8 text-center">
                  ...
                  <p className="text-sm text-muted-foreground">{t("profile.noAppointments")}</p>
                </section>
              ) : (
                    ...
                    return (
                      <div key={a.id} className="bg-card rounded-2xl border border-border p-4">
                        ...
                        <div className="mt-3 flex items-center justify-between text-xs text-muted-foreground">
                          ...
                          <span>{a.visit_type === "online" ? t("profile.visitOnline") : t("profile.visitInPerson")}</span>
                        </div>
                        {a.status !== "cancelled" && a.status !== "completed" && (
                          <button onClick={() => cancelAppointment(a.id)} className="mt-3 w-full bg-destructive/10 text-destructive rounded-xl py-2 text-xs font-bold flex items-center justify-center gap-1">
                            <X className="w-3.5 h-3.5" /> {t("profile.cancelAppointment")}
                          </button>
                        )}
                      </div>
                    );
